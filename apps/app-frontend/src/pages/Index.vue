...
import { profile_listener, process_listener } from '@/helpers/events'
import { useBreadcrumbs } from '@/store/breadcrumbs'
import { handleError } from '@/store/notifications.js'
import dayjs from 'dayjs'
import { get_search_results, get_project, get_version_many } from '@/helpers/cache.js'
...
const getFeaturedModpacks = async () => {
  ...
  if (response?.result?.hits) {
    ...
    featuredModpacks.value = response.result.hits.map((hit, index) => {
      const profile = profiles.find((p) => p.linked_data?.project_id === hit.project_id)
    ...
    try {
      ...
      if (project?.versions?.length) {
        ...
        if (profile?.path && latestVersion?.id && profile?.linked_data?.version_id) {
          await update_managed_modrinth_version(profile.path, latestVersion.id)
          hasUpdate.value[projectId] = false
          await getInstances()
        }
      }
